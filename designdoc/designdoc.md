...
1. Инициализация
    ...
    1.   При инициализировании плагинов рекомендательной логики каждый плагин шлет сообщение в кафка-брокер (топик "started-plugins") о своей инициализации и ip/port сокета, через который с ним может взаимодействовать backend. При выполнении команды docker stop ... контейнер с тем или иным плагином должен послать сообщение в брокер (топик "stopped-plugins") об остановке работы.
        Формат сообщений (plugin_name = имя контейнера, так как в случае MVP предполагается использование docker-compose в качестве оркестратора, label - имя, отображаемое на фронте, а size - размер максимальной стороны изображения в пикселях для его сжатия на стороне фронта):
         ```
         # start-plugins:
         {"plugin_name" : "plugin1", "label" : "плагин 1", "uid", "size": 640 } 
         # stop-plugins:
         {"plugin_name" : "plugin1"}
         ```
...
2. Загрузка входных данных
    ...
    1. Видео кадрируется на фронтенд части и отправляется на бэкенд чанками по 50 фотографий в виде зип архивов. Дополнительно к сообщениям прикреплена информация о UID пользователя и названии видеофайла, который подгружается, а также имя выбранного плагина рекомендательной системы.
    ...
    3. По завершению процессов загрузки всех чанков от фронтенда поступает оповещение данного факта на бэкенд. Бэкенд формирует сообщение о завершении загрузки файла в топик {имя плагина}-new-files. Формат:
