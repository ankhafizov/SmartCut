...
1 - Инициализация
    ...
    1.2 - Backend переодически (1 каждые 5 секунд) считывает сообщения с топиков start/stop и составляет список доступных плагинов и из этой информации формируется фронтенд часть

2 - Загрузка входных данных

    2.1 - Видео кадрируется на фронтенд части и отправляется на бэкенд чанками по 50 фотографий в виде зип архивов. Дополнительно к сообщениям прикреплена информация о UID пользователя и названии видеофайла, который подгружается, а также имя выбранного плагина рекомендательной системы.
    2.2 - Полученные архивы складываются в директорию вида {UUID запроса пользователя}/{basename видео} (например для видео video_name1.mp4: "40b1fea2-4a91-11ee-be56-0242ac120002/video_name1"), которые являются docker shared volume и доступны для контейнеров бэкенда и плагинов. UUID запроса можно генерировать на фронте при начале загрузки видео, basename видеофайла необходим для отладки системы в случае аномального поведения.
    
    ** TODO: проработать случаи, чтобы у пользователя не было доступа к одновременной обработке больше N файлов **
    2.3 - По завершению процессов загрузки всех чанков от фронтенда поступает оповещение данного факта на бэкенд. Бэкенд формирует сообщение о завершении загрузки файла в топик {имя плагина}-new-files. Формат:

    ```
    {"status" : "uploaded", "zipped_images_path" : "40b1fea2-4a91-11ee-be56-0242ac120002/video_name1" }
    ```

3 - Инференс рекомендательной логики

    3.1 - Каждый плагин считывают топики {имя плагина}-new-files
    3.2 - В случае новых сообщений - плагин отрабатывает логику с указанным в сообщении zipped_images_path
    3.3 - На выходе плагин кладет сообщение с временными метками релевантных отрезков в секундах в формате:
    ```
    {
        "user_id" : "user_id1"
    ...
    ```
    в топик processed-files.

4 - Отображение результатов

    4.1 - Бэкенд считывает топик processed-files и роутит полученную информацию к соответствующему клиенту на соответствующее окно браузера.
    4.2 - Фронтенд получает временные отрезки, визуализирует их, предоставляет пользователю возможность редактирования. При нажатии кнопки "сохранить" или ее аналога - нарезает отрезки видео в соответствии с полученными временными метками.


Список всех kafka топиков:
