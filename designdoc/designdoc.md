...
2. Загрузка входных данных
    ...
    1. Видео кадрируется на фронтенд части и отправляется на бэкенд чанками по 50 фотографий jpg формата в виде зип архивов. Дополнительно к сообщениям прикреплена информация о UID пользователя и названии видеофайла, который подгружается, а также имя выбранного плагина рекомендательной системы.

    2. Полученные архивы складываются в директорию вида {UUID запроса пользователя}/{basename видео} (например для видео video_name1.mp4: "40b1fea2-4a91-11ee-be56-0242ac120002/video_name1"), которые являются docker shared volume (название temp_data) и доступны для контейнеров бэкенда и плагинов. UUID запроса можно генерировать на фронте при начале загрузки видео, basename видеофайла необходим для отладки системы в случае аномального поведения.
    ...
    3. При загрузке каждого чанка за исключением последнего в кафку в топик {имя плагина}-new-files шлются сообщения. Формат сообщений:
        ...
        ```
        {
            "status" : "in-progress",
            "user_id" : "user_id1",
            "last_zipped_chunk_path" : "40b1fea2-4a91-11ee-be56-0242ac120002/video_name1/0.zip"
        }
        ```
...
3. Инференс рекомендательной логики
    ...
    3. На выходе плагин кладет сообщение в топик processed-files: 
        - в случае обработки чанка - уведомление о завершении:
       ```
        {
            "user_id" : "user_id1", 
            ...
            "plugin_name" : "plugin1",  
            "processed_chunk": "0.zip" 
        } 
       ```
        - в случае обработки всех чанков - сообщение с временными метками релевантных отрезков в секундах в формате:
       ```
       {
           "user_id" : "user_id1",
           "user_request_uid" : "40b1fea2-4a91-11ee-be56-0242ac120002",
       ...
       ```
    4. Обработанные файлы удаляются из хранилища.
