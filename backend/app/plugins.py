...
class PluginsManager:
    ...
    def __init__(self):
        ...
        # Продюсер Kafka для отправки сообщений плагинам через Kafka
        self.kafka_producer = KafkaProducer(
            bootstrap_servers=os.getenv("KAFKA_BOOTSTRAP_SERVER"),
            value_serializer=lambda x: dumps(x).encode("utf-8")
        )

        # Консьюмер Kafka для получения сообщений от плагинов
        self.kafka_consumer = KafkaConsumer(
            bootstrap_servers=os.getenv("KAFKA_BOOTSTRAP_SERVER"),
            value_deserializer=lambda x: loads(x.decode("utf-8")),
            consumer_timeout_ms=int(os.getenv("KAFKA_POLL_PERIOD"))*1000
        )
        # Подписываемся сразу на все нужные топики
        self.kafka_consumer.subscribe(["start-plugins", "processed-files"])
    ...
    def run(self):
        ...
        :return:
        """
        thread = Thread(target=self.__kafka_poll)
        thread.start()
    ...
    def __kafka_poll(self):
        """
        Функция обновляет список плагинов и результатов запросов пользователей из Kafka
        :return:
        """
        while True:
            if stop_flag:
            ...
            try:
                for message in self.kafka_consumer:
                    if message.topic == "start-plugins" and message.value.get("plugin_name"):
                        self.plugins[message.value["plugin_name"]] = message.value
                    elif message.topic == "processed-files" and message.value.get("user_request_uid"):
